...
class ARC
{
    ...
    public function getPlayersArray()
    {
        ...
        $formatList = $this->formatList($players);
    ...
    public function formatList($str) {
        ...
        foreach ($line as $lines) {
            ...
            if ($lines == 'Players on server:')
                continue;

            if ($lines == '[#] [IP Address]:[Port] [Ping] [GUID] [Name]')
                continue;

            if ($lines == '--------------------------------------------------')
                continue;

            if (strpos($lines, 'players in total)') !== false)
                continue;


            if (!preg_match("#^([0-9]+)\s+([0-9\.]+):([0-9]+)\s+([0-9]+)\s+([a-f0-9]+)(\(OK\))?\s+(.*)$#im", $lines)) {
                throw new \InvalidArgumentException("Input string is not valid");
            }

            preg_match("#^([0-9]+)\s+([0-9\.]+):([0-9]+)\s+([0-9]+)\s+([a-f0-9]+)(\(OK\))?\s+(.*)$#im", $lines, $matches);
